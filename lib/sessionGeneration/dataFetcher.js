...
export async function obtenerDatosContextuales(userId) {
    ...
    try {
        ...
        return {
            ...
            recentSessions: userDoc.recentSessions || [], // Added to support consistency logic
...
async function obtenerDocumentoUsuario(userId) {
    ...
    return {
        ...
        recentSessions: userData.recentSessions || [], // Added to fetch recent sessions
...
/**
 * Obtiene historial de un ejercicio específico, priorizando el ambiente actual
 * Si el ambiente cambió (gym→home o viceversa), busca en sesiones anteriores con mismo ambiente
 */
export function getExerciseHistory(history, exerciseId, limit = 10, currentEnvironment = null) {
    if (!history || history.length === 0) return [];
    
    const exerciseRecords = [];
    const sameEnvironmentRecords = [];
    const differentEnvironmentRecords = [];
    
    for (const session of history) {
        // Aceptar distintos formatos: mainBlocks | mainBlock | coreBlocks | coreBlock
        const mainBlocks = session.mainBlocks || (session.mainBlock ? (Array.isArray(session.mainBlock) ? session.mainBlock : [session.mainBlock]) : []);
        const coreBlocks = session.coreBlocks || (session.coreBlock ? (Array.isArray(session.coreBlock) ? session.coreBlock : [session.coreBlock]) : []);
        const allBlocks = [ ...mainBlocks, ...coreBlocks ];

        if (!allBlocks || allBlocks.length === 0) continue;

        for (const block of allBlocks) {
            // Soportar ejercicios bajo 'exercises', 'ejercicios' o 'estaciones' con 'ejercicios'
            let ejerciciosArr = [];
            if (Array.isArray(block.exercises)) ejerciciosArr = block.exercises;
            else if (Array.isArray(block.ejercicios)) ejerciciosArr = block.ejercicios;
            else if (Array.isArray(block.estaciones)) {
                block.estaciones.forEach(st => {
                    if (Array.isArray(st.ejercicios)) ejerciciosArr.push(...st.ejercicios);
                });
            }
            else if (Array.isArray(block.bloques)) {
                block.bloques.forEach(bl => {
                    if (Array.isArray(bl.ejercicios)) ejerciciosArr.push(...bl.ejercicios);
                });
            }

            const exercise = ejerciciosArr.find(e => e.id === exerciseId);
            if (exercise && exercise.performanceData) {
                const sessionEnvironment = session.sessionEnvironment || session.meta?.location || null;
                const record = {
                    date: session.meta?.date || session.completedAt || session.date,
                    sessionFocus: session.meta?.focus || session.sessionFocus,
                    ...exercise,
                    sessionFeedback: session.feedback,
                    // Propagar metadatos de ambiente/equipo si existen
                    sessionEnvironment: sessionEnvironment,
                    equipmentSnapshot: session.equipmentSnapshot || null
                };
                
                // Separar por ambiente si currentEnvironment está especificado
                if (currentEnvironment && sessionEnvironment) {
                    if (sessionEnvironment === currentEnvironment) {
                        sameEnvironmentRecords.push(record);
                    } else {
                        differentEnvironmentRecords.push(record);
                    }
                } else {
        ...
        // Si ya tenemos suficientes del mismo ambiente, podemos parar
        if (currentEnvironment && sameEnvironmentRecords.length >= limit) break;
        if (!currentEnvironment && exerciseRecords.length >= limit) break;
    }
    
    // Priorizar registros del mismo ambiente, luego otros ambientes
    if (currentEnvironment) {
        const combined = [...sameEnvironmentRecords, ...differentEnvironmentRecords];
