...
// Mapeo de músculos a estiramientos recomendados
const STRETCHES_BY_MUSCLE = {
    // Tren Superior
    pecho: ['estiramiento_pectoral_pared', 'estiramiento_pectoral_suelo', 'apertura_brazos_suelo'],
    hombros: ['estiramiento_hombro_cruzado', 'estiramiento_deltoides', 'rotacion_hombro_pared'],
    espalda: ['child_pose', 'gato_vaca', 'estiramiento_lat_pared', 'torsion_espinal_suelo'],
    biceps: ['estiramiento_biceps_pared', 'estiramiento_biceps_puerta'],
    triceps: ['estiramiento_triceps_overhead', 'estiramiento_triceps_pared'],
    
    // Tren Inferior
    cuadriceps: ['estiramiento_cuadriceps_pie', 'estiramiento_cuadriceps_suelo', 'estiramiento_recto_femoral'],
    isquiotibiales: ['estiramiento_isquio_pie', 'estiramiento_isquio_suelo', 'good_morning_stretch'],
    gluteos: ['estiramiento_piriforme', 'estiramiento_gluteo_suelo', 'figura_4_suelo'],
    pantorrillas: ['estiramiento_soleo_pared', 'estiramiento_gastrocnemio_pared', 'bajada_talon_escalon'],
    
    // Flexores de cadera
    hip_flexors: ['estiramiento_psoas_rodilla', 'estiramiento_recto_femoral', 'lunge_stretch', 'couch_stretch'],
    
    // Core
    core: ['child_pose', 'cobra_stretch', 'gato_vaca', 'torsion_espinal_suelo']
};
...
export function generarEnfriamiento(ejerciciosEnfriamiento, bloquePrincipal, nivel, duracionMaxima = 8) {
    ...
    // ====================================================================
    const estiramientosEspecificos = seleccionarEstiramientos(musculosTrabajados, nivel);
    ...
    // ====================================================================
    // AÑADIR EJERCICIOS DEL CATÁLOGO SI HAY DISPONIBLES
    // ====================================================================
    if (ejerciciosEnfriamiento && ejerciciosEnfriamiento.length > 0) {
        incorporarEjerciciosCatalogo(enfriamiento, ejerciciosEnfriamiento, nivel);
    }
...
/**
 * Selecciona estiramientos específicos para los músculos trabajados
 */
function seleccionarEstiramientos(musculos, nivel) {
    const estiramientos = [];
    ...
    for (const musculo of musculos) {
        const disponibles = STRETCHES_BY_MUSCLE[musculo];
        if (disponibles && disponibles.length > 0) {
            // Tomar el primero (más básico)
            const stretch = disponibles[0];
            
            // Evitar duplicados
            if (!estiramientos.find(e => e.id === stretch)) {
                estiramientos.push({
                    id: stretch,
                    nombre: formatearNombreEstiramiento(stretch),
                    tiempo: `${tiempoBase}s`,
                    musculoObjetivo: musculo,
                    instrucciones: getInstruccionesEstiramiento(stretch)
                });
            }
        }
