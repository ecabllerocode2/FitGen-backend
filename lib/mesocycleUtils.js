...
/**
 * Whether today's session is the last training day of the microcycle week.
 */
export function isLastSessionOfWeek(mesocycle, weekNumber, dayOfWeek) {
  const micro = mesocycle.microcycles?.find((m) => m.week === weekNumber);
  if (!micro?.sessions?.length) return false;

  const trainingDays = micro.sessions
    .filter((s) => !s.isRestDay && s.sessionFocus !== 'Descanso')
    .map((s) => s.dayOfWeek)
    .sort((a, b) => DAY_ORDER.indexOf(a) - DAY_ORDER.indexOf(b));

  if (!trainingDays.length) return false;
  return dayOfWeek === trainingDays[trainingDays.length - 1];
}
